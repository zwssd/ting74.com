    ...
    def start_requests(self):
        if self.test_model:
            ...
            for ph_type in self.start_urls:
                ...
                #print("insert db end......")
                yield Request(url=self.host + ph_type, callback=self.parse_anthology)

    def parse_anthology(self, response):
        #logging.debug('response:------>' + response.text)
        #sys.exit(0)
        selector = Selector(response)
        #logging.debug('request url:------>' + response.url)
        #logging.info(selector)
        lis = selector.xpath('//div[@class="playlist"]//a')
        for li in lis:
            logging.debug('li :------>' + li.extract())
            viewkey = re.findall('href="(.*?)"', li.extract())
            logging.debug('viewkey :------>' + viewkey[0])
            self.pipelines.insert_link("ITEM", self.host + viewkey[0])
            yield Request(url=self.host + viewkey[0],
                          callback=self.parse_play_address)
